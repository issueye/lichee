...
// 生成RSA key
func (r RSA) RSAGenKey(bits int) ([]byte, []byte, error) {
	var (
		privateBytes []byte
		publicBytes  []byte
	)
...
// 从文件中读取RSA key
func (r RSA) RSAReadKeyFromFile(filename string) []byte {
...
// RSA加密
func (r RSA) RSAEncrypt(data, publicBytes []byte) ([]byte, error) {
	var res []byte
	// 解析公钥
	...
	// 将数据加密为base64格式
	return []byte(Crypt{}.EncodeBase64(string(res))), nil
}

// 对数据进行解密操作
func (r RSA) RSADecrypt(base64Data, privateBytes []byte) ([]byte, error) {
	var res []byte
	// 将base64数据解析
	data := []byte(Crypt{}.DecodeBase64(string(base64Data)))
	// 解析私钥
